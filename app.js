...
function home() {
  ...
  const insights = c.articles.filter(a => a.status === 'Publicado');
...
function articlesView() { const articles = getContent().articles; return `<section class="admin-content"><div class="admin-head"><div><h1>Conteúdos</h1><p>Gerencie os artigos exibidos na página “Conteúdos” e nos destaques da home.</p></div><div class="admin-actions"><button class="btn btn--primary" data-admin-new-article>Novo artigo</button></div></div><section class="admin-card"><div class="admin-card__head"><h2>Artigos</h2></div><table class="table"><thead><tr><th>Título</th><th>Categoria</th><th>Data</th><th>Status</th><th></th></tr></thead><tbody>${articles.map((a,i)=>`<tr><td><strong>${esc(a.title)}</strong></td><td>${esc(a.category)}</td><td>${esc(a.date)}</td><td><span class="status status--${a.status==='Publicado'?'done':'draft'}">${esc(a.status)}</span></td><td class="table-actions"><button class="text-link" data-admin-edit-article="${i}">Editar</button><button class="text-link text-link--danger" data-admin-del-article="${i}">Excluir</button></td></tr>`).join('')}</tbody></table></section></section>`; }

function articleEditView(index) { const articles = getContent().articles; const isNew = index === null || index === undefined || Number.isNaN(index) || !articles[index]; const a = isNew ? { title:'', category:'Institucional', date:'', status:'Rascunho', summary:'' } : articles[index]; const cats = ['Institucional','Regulação','Experiência','Gestão']; return `<section class="admin-content"><div class="admin-head"><div><h1>${isNew?'Novo artigo':'Editar artigo'}</h1><p>As alterações são salvas neste navegador e refletem no site imediatamente.</p></div><div class="admin-actions"><button class="btn btn--line" data-admin-view="artigos">Cancelar</button></div></div><form class="admin-form" id="article-form" data-index="${isNew?'':index}"><div class="field"><label for="a-title">Título</label><input id="a-title" required value="${esc(a.title)}" placeholder="Insira um título"></div><div class="admin-form__grid"><div class="field"><label for="a-category">Categoria</label><select id="a-category">${cats.map(cat=>`<option ${cat===a.category?'selected':''}>${cat}</option>`).join('')}</select></div><div class="field"><label for="a-status">Status</label><select id="a-status"><option ${a.status==='Publicado'?'selected':''}>Publicado</option><option ${a.status==='Rascunho'?'selected':''}>Rascunho</option></select></div></div><div class="admin-form__grid"><div class="field"><label for="a-date">Data (texto)</label><input id="a-date" value="${esc(a.date)}" placeholder="ex.: 21 jul 2026"></div></div><div class="field"><label for="a-summary">Resumo</label><textarea id="a-summary" placeholder="Introdução objetiva para a publicação.">${esc(a.summary||'')}</textarea></div><button class="btn btn--primary" type="submit">Salvar artigo</button></form></section>`; }
